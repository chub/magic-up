  ...
  if(fs.existsSync(mupJsonPath)) {
    ...
    if(!mupJson.servers || mupJson.servers.length == 0) {
      ...
      mupJson.servers.forEach(function(server) {
        ...
        // If there is no SSH Agent running, and no pem specified, use id_rsa if it exists.
        if (!sshAgentExists && !server.pem) {
          if (fs.existsSync(rewriteHome('~/.ssh/id_rsa'))) {
            server.pem = "~/.ssh/id_rsa";
          }
        }
